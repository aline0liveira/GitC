        ...
        private void DataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex > -1)
            {
                ...
                switch (e.ColumnIndex)
                {
                    ...
                    case 2:
                        {
                            var dialogResult = MessageBox.Show("Deseja realmente ajustar esse valor?"
                                , "Edição"
                                ...
                                if(dialogResult == DialogResult.Yes)
                            {
                                ...
                                if (dataInformada <= DateTime.Now)
                                    listContracs.FirstOrDefault(x =>
                                    x.Id == (int)collumId.Value).DatInc = DateTime.Parse(collValue.ToString());
                                else
                                    MessageBox.Show("Não foi possível alterar o resgistro de data");
